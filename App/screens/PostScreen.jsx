...
// }, [route.params?.markerCoordinate]);
  ...
  const handlePost = async () => {
    ...
    console.log(imageListURL,'imageListURLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL');
    ...
    const docRef = await addDoc(collection(db, "posts"), {
      ...
      await setDoc(docRefU, {
                    ...
                    myPosts: [
                      ...
                      }, ...dataSnapshotU.myPosts
                    ]
                      
                    
                  })
                  const q = query(collection(db, "posts"), orderBy('timestamp', 'desc'));
                  const querySnapshotP = await getDocs(q);
                  const dataSnapshotP = querySnapshotP.docs.map((doc) => ({ postId: doc.id, ...doc.data()}));
                  const myPosts = dataSnapshotP.filter((post) => post.userId === mockAuth.currentUser.uid);

                  setNewPost(myPosts);

                  const timestamp1 = myPosts.map((post) => post.timestamp);
                  // console.log(timestamp1, 'this is timestamp')

                  // console.log(myPosts.timestamp, 'this is myPosts timestamp')


                  setNameOfCoWorkingSpace('');
                  setAddressOfCoWorkingSpace('');
                  setDescription('');
                  setAmenities([]);
                  setAvailableSpaces([]);
                  setImageList([]);
                  setMarkerCoordinate(null);
                  setEmailOfCoWorkingSpace('');
                  setPhoneNumberOfCoWorkingSpace('');
                  setFacebookLinkOfCoWorkingSpace('');
                  setInstagramLinkOfCoWorkingSpace('');
                  setXTwitterLinkOfCoWorkingSpace('');
                  setLineLinkOfCoWorkingSpace('');
                  ...
                  setIsSelectedSaturday(false);
                  setIsSelectedSunday(false);

                  setIsPressAddPhotos(false);
                  setIsPressAddContact(false);
  ...
  // const [sundayCloseTime, setSundayCloseTime] = useState({time: 'Open 24 hours', timeFromMidnight: 'Open 24 hours'});
                  ...
                  // })
                  setIsLoading(false);
                  navigation.navigate('HomeScreen', markerCoordinate);
...
<>{isLoading ? <LoadingScreen /> :
    <View style={{flex:1,backgroundColor: isDarkTheme ? '#150228' : '#ddddff'}}>
        ...
        <View style={{alignItems: 'center',width: '100%',height: '100%',}}>
            ...
            <Text style={{fontSize: h(4),color: isDarkTheme ? 'white' : 'black',fontWeight: 'bold',fontFamily: 'Rockwell', paddingTop: h(8),shadowOffset: { width: 0, height:  0},shadowOpacity: 0,}}>Post</Text>
...
<View style={{width: '100%',padding: h(1.5),marginTop: h(1),backgroundColor: 'rgba(255, 255, 255, 1)',borderRadius: h(2),}}>
...
<View style={{width: '100%',paddingTop: h(2)}}>
            ...
            <View style={{marginTop: h(15),width: w(80),alignItems: 'center',marginBottom: h(30)}}>
                <TouchableOpacity onPress={handlePost} style={{borderColor: 'white',borderRadius: h(2.6),backgroundColor: isDarkTheme ? '#99FFFF' : '#473FB7',width: '100%',height: h(6),justifyContent: 'center',alignItems: 'center'}}>
                    <Text style={{color: isDarkTheme ? 'black' : 'white',fontSize: h(3),fontFamily: 'PatrickHand-Regular'}}>Post now!</Text>
                </TouchableOpacity>
